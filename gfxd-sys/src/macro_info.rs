...
extern "C" {
    ...
    ///
    /// If the macro does not have a name (i.e. it's invalid), null is returned.
    ///
    ...
    ///
    /// The return value will be `gfxd_argfmt_i` for `int32_t`, `gfxd_argfmt_u`
    /// for `uint32_t`, or `gfxd_argfmt_f` for `float`.
    ///
    ...
    ///
    /// An `idx` of zero returns the first argument that has the specified type.
    ///
